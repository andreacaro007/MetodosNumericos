...
import { EntradaFuncion, type MetodoDisponible, type SolicitudResolucion } from '../../caracteristicas/entrada-funcion/entrada-funcion';
...
export class Calculadora implements OnDestroy {
  ...
  cambiarMetodo(metodo: MetodoDisponible): void { this.detener(); this.metodo.set(metodo); this.resultado.set(undefined); this.error.set(undefined); this.indiceActivo.set(0); this.pestana.set('resumen'); }
  anterior(): void { this.indiceActivo.update((indice) => Math.max(0, indice - 1)); }
  siguiente(): void { const ultimo = (this.resultado()?.iteraciones.length ?? 1) - 1; this.indiceActivo.update((indice) => Math.min(ultimo, indice + 1)); }
  ...
  reproducir(): void {
    ...
    this.temporizador = setInterval(() => { const ultimo = (this.resultado()?.iteraciones.length ?? 1) - 1; if (this.indiceActivo() >= ultimo) this.detener(); else this.siguiente(); }, 850);
  }
