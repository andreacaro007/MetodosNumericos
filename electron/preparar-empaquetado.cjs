...
const motorPython = path.join(raiz, "python", "dist", "motor-numerico.exe");
...
if (!fs.existsSync(motorPython)) {
  throw new Error("No existe el motor Python. Ejecuta npm run compilar:python antes de empaquetar.");
}
...
fs.copyFileSync(motorPython, path.join(destino, "motor-numerico.exe"));
fs.writeFileSync(path.join(destino, 'package.json'), JSON.stringify({
  ...
  build: {
    ...
    files: ['**/*'],
    extraResources: [{ from: "motor-numerico.exe", to: "motor-numerico.exe" }],
